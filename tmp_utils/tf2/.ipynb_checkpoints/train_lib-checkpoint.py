...
class DisplayCallback(tf.keras.callbacks.Callback):
  ...
  def set_model(self, model: tf.keras.Model):
    self.model = model
    config = model.config
    log_dir = os.path.join(config.model_dir, 'data')
    self.file_writer = tf.summary.create_file_writer(log_dir)
...
def get_callbacks(params, val_dataset=None):
  ...
  if params['moving_average_decay']:
    ...
    ckpt_callback = tf.keras.callbacks.ModelCheckpoint(
        os.path.join(params['log_dir'], 'ckpt-{epoch:d}'),
        verbose=params['verbose'],
        save_freq=params['save_freq'],
        save_weights_only=True)
  ...
  tb_callback = tf.keras.callbacks.TensorBoard(
      ...
      profile_batch=0) #2 if params['profile'] else 0
  callbacks.append(tb_callback)
  if params.get('sample_image', None):
    display_callback = DisplayCallback(
        params.get('sample_image', None), params['log_dir'],
